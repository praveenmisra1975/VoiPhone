...
static void on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
      ...
      PJ_LOG(5,(THIS_FILE, "Call %d state=%.*s", call_id,
           ...
           pjsip_rx_data *rdata)
{
    ...
    PJ_LOG(5,(THIS_FILE, "Incoming call from %.*s!!",
...
static void sip_manage_codec(void)
{
  pj_status_t status;
  unsigned i;
  ...
  for (i = 0; i < count; ++i) 
  {
   ...
   // {
      status = pjsua_codec_set_priority(&codecs[i].pjsip_name, PJMEDIA_CODEC_PRIO_DISABLED);
        ...
        PJ_LOG(5,(THIS_FILE, " %s codec priority (Status. %d)",
                  codecs[i].pjsip_name, status));
        
      if (status != PJ_SUCCESS)
        PJ_LOG(5,(THIS_FILE, "Error setting %s codec priority (Err. %d)", 
                  codecs[i].pjsip_name, status));
   // }
  }
}
...
pj_status_t sip_startup(app_config_t *app_config)
{
  ...
  val = [[NSUserDefaults standardUserDefaults] integerForKey: 
    ...
    app_config->log_cfg.msg_logging =  PJ_TRUE;
 ...
 */
  ...
  pjsua_media_config_default(&(app_config->media_cfg));
    ...
    app_config->cfg.hangup_forked_call=1;
...
//  if (![[NSUserDefaults standardUserDefaults] boolForKey:@"enableEC"])
 ...
 // app_config->media_cfg.enable_ice = [[NSUserDefaults standardUserDefaults] boolForKey:@"enableICE"];
  ...
  */
  //app_config->udp_cfg.port = val;
   ...
   // PJ_LOG(5,(THIS_FILE, "SIP Port : " + app_config->udp_cfg.port));
   
  pjsua_transport_config_default(&(app_config->rtp_cfg));
    
 // app_config->rtp_cfg.port = [[NSUserDefaults standardUserDefaults] integerForKey: @"rtpPort"];
    
    app_config->rtp_cfg.port=10000;
...
/*
  ip_addr = [[[NSUserDefaults standardUserDefaults] stringForKey: @"boundAddr"] UTF8String];
  ...
  ip_addr = [[[NSUserDefaults standardUserDefaults] stringForKey: @"publicAddr"] UTF8String];
  if (ip_addr && strlen(ip_addr))
  {
    pj_strdup2_with_null(app_config->pool, 
...
/*  srv = [[[NSUserDefaults standardUserDefaults] stringForKey: @"stunServer"] UTF8String];
  if (srv && strlen(srv))
  {
    if (app_config->cfg.stun_srv_cnt==PJ_ARRAY_SIZE(app_config->cfg.stun_srv)) 
    {
      PJ_LOG(5,(THIS_FILE, "Error: too many STUN servers"));
      return PJ_ETOOMANY;
    }
    pj_strdup2_with_null(app_config->pool, 
 ...
 */
  ...
  //app_config->cfg.outbound_proxy[0] = pj_str(Domain);
 ...
 /* dns = [[NSUserDefaults standardUserDefaults] stringForKey: @"dnsServer"];
  ...
  while (anObject = [enumerator nextObject])
  {
    ...
    if (srv && strlen(srv))
    {
      ...
      {
        PJ_LOG(5,(THIS_FILE, "Error: too many DNS servers"));
        [mutableStr release];
        break;
      }
      pj_strdup2_with_null(app_config->pool, 
  ...
  if (status != PJ_SUCCESS)
    ...
    app_config->tcp_cfg.port=5060;
 status = pjsua_transport_create(PJSIP_TRANSPORT_TCP, &app_config->tcp_cfg, &transport_id);
if (status != PJ_SUCCESS)
    goto error;
    ...
    app_config->tcp_cfg.tls_setting.qos_type = app_config->tcp_cfg.qos_type;
    pj_memcpy(&app_config->tcp_cfg.tls_setting.qos_params, &app_config->tcp_cfg.qos_params,
          sizeof(app_config->tcp_cfg.qos_params));

    // Set TLS port as TCP port+1
   
        app_config->tcp_cfg.port++;
...
pj_status_t sip_connect(pj_pool_t *pool, pjsua_acc_id *acc_id)
{
    ...
    acc_cfg.contact_params = pj_str(";+sip.instance=\"<urn:uuid:1f11f215-40dd-42fb-9896-59a22ad9cgud>\"");;
    
    // ID
    acc_cfg.id.ptr = (char*) pj_pool_alloc(pool, PJSIP_MAX_URL_SIZE);
    ...
    {
        PJ_LOG(5,(THIS_FILE, "Error: invalid SIP URL '%s' in local id argument",
                  acc_cfg.id));
        [app displayParameterError: @"Invalid value for username or server."];
        return status;
    }
   PJ_LOG(5,(THIS_FILE,  "SIP id '%s' in registrar argument",
   acc_cfg.id));
    
    // Registrar
    acc_cfg.reg_uri.ptr = (char*) pj_pool_alloc(pool,
                                                PJSIP_MAX_URL_SIZE);
    acc_cfg.reg_uri.slen = pj_ansi_snprintf(acc_cfg.reg_uri.ptr,
                                            PJSIP_MAX_URL_SIZE, "sip:%s", server);
    PJ_LOG(5,(THIS_FILE,  "SIP URL '%s' in registrar argument",
              acc_cfg.reg_uri));
    
    
    if ((status = pjsua_verify_sip_url(acc_cfg.reg_uri.ptr)) != 0)
    {
        PJ_LOG(5,(THIS_FILE,  "Error: invalid SIP URL '%s' in registrar argument",
                  acc_cfg.reg_uri));
...
//#endif
   ...
   // acc_cfg.allow_contact_rewrite = [[NSUserDefaults standardUserDefaults] boolForKey:@"UseNAT"];
    
    // FIXME: gestion du message 423 dans pjsip
  //  acc_cfg.reg_timeout = [[NSUserDefaults standardUserDefaults] integerForKey: @"ReregistrationTime"];
    acc_cfg.reg_timeout =1600;
   ...
   // acc_cfg.ka_interval = [[NSUserDefaults standardUserDefaults] integerForKey: @"kaInterval"];
    ...
    // proxies server
  /*  NSString *proxies = [[NSUserDefaults standardUserDefaults] stringForKey: @"OutboundProxies"];
    ...
    while (anObject = [enumerator nextObject])
    {
        ...
        if (proxy && strlen(proxy))
        {
            ...
            {
                PJ_LOG(5,(THIS_FILE, "Error: too many proxy servers"));
                [mutableStr release];
                break;
            }
            pj_str_t pj_proxy;
            ...
            {
                PJ_LOG(5,(THIS_FILE,  "Error: invalid SIP URL '%*.s' in proxy argument (%d)",
 ...
 pj_str_t pj_proxy = pj_str((char *)[[[NSUserDefaults standardUserDefaults] stringForKey: @"Domain"] UTF8String]);
    if (pj_strlen(&pj_proxy) > 0)
    {
  acc_cfg.proxy[0].ptr = (char*) pj_pool_alloc(pool,PJSIP_MAX_URL_SIZE);
  acc_cfg.proxy[0].slen = pj_ansi_snprintf(acc_cfg.proxy[0].ptr,
        PJSIP_MAX_URL_SIZE, "sip:%s;transport=udp;lr", pj_proxy);
          // FIXME verify in settings view
  if ((status = pjsua_verify_sip_url(acc_cfg.proxy[0].ptr)) != 0)
    {
       PJ_LOG(5,(THIS_FILE,  "Error: invalid SIP URL '%s' in proxy argument", acc_cfg.reg_uri));
            [app displayParameterError: @"Invalid value for proxy parameter."];
            return status;
          }
          acc_cfg.proxy_cnt = 1;
  ...
  //  acc_cfg.srtp_optional_dup_offer = PJ_TRUE;
    ...
    if (pjsua_acc_is_valid(*acc_id))
    {
      ...
      if (status != PJ_SUCCESS)
      {
          pjsua_perror(THIS_FILE, "Error modifing account", status);
          
        /*  status = pjsua_acc_add(&acc_cfg, PJ_TRUE, acc_id);
          if (status != PJ_SUCCESS)
          {
            *acc_id = PJSUA_INVALID_ID;
            pjsua_perror(THIS_FILE, "Error adding new account in valid case", status);
            [app displayParameterError: @"Error adding new account in valid case."];
          }
         */
      }
    }else
    {
        ...
        {
          *acc_id = PJSUA_INVALID_ID;
          pjsua_perror(THIS_FILE, "Error adding new account invalid case", status);
          [app displayParameterError: @"Error adding new account invalid case."];
        }
...
//  pj_str_t hvalue, hname;
  ...
  if (status != PJ_SUCCESS) 
  {
    PJ_LOG(5,(THIS_FILE,  "Invalid URL \"%s\".", uri));
...
{
  pj_status_t status;
    NSLog(@"call_id while ansder:  %d", call_id);
...
pj_status_t sip_add_account(NSDictionary *account, pjsua_acc_id *acc_id)
{
  ...
  if (pj_strlen(&pj_proxy) > 0)
  {
    ...
    if ((status = pjsua_verify_sip_url(acc_cfg.proxy[0].ptr)) != 0) 
    {
      PJ_LOG(5,(THIS_FILE,  "Error: invalid SIP URL '%s' in proxy argument",
                acc_cfg.reg_uri));
      [app displayParameterError: @"Invalid value for proxy parameter."];
      return status;
    }
