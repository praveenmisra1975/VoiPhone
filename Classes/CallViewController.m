...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil 
{
	...
	if (self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil])
  {
      ...
      widthCall=  myRect.size.width ;
      heightCall= myRect.size.height ;
 ...
 - (void)loadView
{
  ...
  // create the container view which we will use for transition animation (centered horizontally)
	CGRect frame = CGRectMake(0.0f, heightCall*.147, widthCall, heightCall*.666);
	
    _containerView = [[UIView alloc] initWithFrame:frame];
	
  /** Phone Pad **/
  
    PhonePad *phonePad = [[PhonePad alloc] initWithFrame: CGRectMake(0.0f, 0.0f, widthCall, heightCall*.666)];
  [phonePad setPlaysSounds: TRUE];
  [phonePad setDelegate: self];
  
  /** Menu **/
 MenuCallView *menuView = [[MenuCallView alloc] initWithFrame: CGRectMake(widthCall*.19, heightCall*.19, widthCall*.89, heightCall*.45)];
  ...
  _buttonView = [[DualButtonView alloc] initWithFrame: CGRectMake(widthCall*.05, heightCall*.108, widthCall*.89, heightCall*.45)];
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    NSLog(@"Dial app is now diapppearing ");
    int i;
